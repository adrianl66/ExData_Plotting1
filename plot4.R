plot4 <- function() {
  ...
  # set col and row for the 4 plots
  par(mfcol = c(2,2))
  
  #plot graph #1
  plot(df$Date,df$Global_active_power, xlab = "", ylab = "Global Active Power", type = "l")
  ...
  legend("topright", legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), border = FALSE, cex= 0.7, lty = 1, col = c("black","red","blue"))
  
  #plot graph #3
  plot(df$Date,df$Voltage, xlab = "date/time", ylab = "Voltage", type = "l")
  
  
  #plot graph #4
  plot(df$Date,df$Global_reactive_power, xlab = "date/time", ylab = "Global_reactive_power", type = "l")
